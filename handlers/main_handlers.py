...
from settings.config import MY_LOGGER, WRITE_INTERESTS_FORM, BLACK_LIST_FORM
...
async def start_handler(_, update):
    ...
    if write_usr_rslt:
        button_data = (
            ('✏️ Записать интересы', f"{WRITE_INTERESTS_FORM}?tlg_id={update.from_user.id}"),
            ('🏴‍☠️ Черный список', f"{BLACK_LIST_FORM}?tlg_id={update.from_user.id}"),
        )
